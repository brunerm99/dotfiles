#!/usr/bin/env python3
"""Regenerate native app themes from the reviewed Rimeworks palettes (Python 3.11+)."""
import json
...
for mode in ('light', 'dark'):
    ...
    write(f'apps/config/ghostty/themes/Rimeworks{mode.title()}', ghostty)
...
'''
    write(f'apps/config/gtk-3.0/rimeworks/{mode}.css', gtk)
    ...
    write_json(f'apps/vscode/rimeworks-themes/themes/{mode}-color-theme.json', {
    ...
    write_json(f'{theme_dir}/vscode.json', {'name':title,'extension':'rimeworks.rimeworks-themes'})
...
write_json('apps/vscode/rimeworks-themes/package.json', {
    'name':'rimeworks-themes','displayName':'Rimeworks Themes','publisher':'rimeworks',
    'version':'0.1.0','engines':{'vscode':'^1.80.0'},'categories':['Themes'],
    'description':'Rimeworks light and dark palettes with readable terminal and syntax colors.',
    'contributes':{'themes':[{'label':'Rimeworks '+mode.title(),'uiTheme':'vs' if mode=='light' else 'vs-dark',
                             'path':f'./themes/{mode}-color-theme.json'} for mode in ('light','dark')]},
})
write('apps/obsidian/rimeworks.css', '\n'.join(obsidian))
...
'''
write('apps/config/nvim/colors/rimeworks.lua', lua)
for mode in PALETTES:
    write(f'rimeworks-{mode}/nvim/colors/rimeworks.lua', lua)
    write(f'rimeworks-{mode}/neovim.lua', '''-- Omarchy's LazyVim adapter; the standalone Kraken colorscheme needs no plugin.
return {
  { 'LazyVim/LazyVim',
    init = function()
...
fish = '''function rimeworks --description 'Use the Rimeworks terminal palette for Fish'
'''
for key,value in {
...
fish += 'end\n'
write('apps/config/fish/functions/rimeworks.fish', fish)
...
write_json('apps/snippets/vscode-settings.json', {
    'workbench.colorTheme':'Rimeworks Dark','window.autoDetectColorScheme':True,
    'workbench.preferredDarkColorTheme':'Rimeworks Dark','workbench.preferredLightColorTheme':'Rimeworks Light',
