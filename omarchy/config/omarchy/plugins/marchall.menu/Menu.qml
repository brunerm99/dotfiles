...
Item {
  ...
  // The installed shell gives this menu clone a null appLibrary. Use the
  // same desktop-entry engine locally until the host provides it again.
  readonly property var appLibrary: sharedAppLibrary || localAppLibrary.item
  onAppLibraryChanged: {
    if (root.appLibrary && root.providersLoaded["apps"]) root.mergeAppRows()
  }
  Loader {
    id: localAppLibrary
    active: !root.sharedAppLibrary
    sourceComponent: Component {
      Services.AppLibrary { omarchyPath: root.omarchyPath }
    }
  }
  ...
  function confirmDelete() {
    ...
    root.deleteTarget = null
    if (!target) return
    // Keep the menu and filter available for further app removals.
    root.cancelDelete()
