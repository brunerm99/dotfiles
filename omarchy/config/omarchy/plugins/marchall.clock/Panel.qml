...
Panel {
  id: root
  moduleName: "marchall.clock"
  ipcTarget: "marchall.clock"
  manageIpc: false
  ...
  property bool daySchedule: false
  property date scheduleDate: new Date()

  function moveDay(delta) {
    scheduleDate = new Date(scheduleDate.getFullYear(), scheduleDate.getMonth(), scheduleDate.getDate() + delta)
  }
  ...
  function refresh() {
    root.today = new Date()
    root.daySchedule = false
    root.goToToday()
  }

  function goToToday() {
    root.scheduleDate = new Date()
  ...
  KeyboardPanel {
    ...
    contentHeight: panel.fittedContentHeight(root.daySchedule ? scheduleColumn.implicitHeight : calendarColumn.implicitHeight)
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        if (root.daySchedule) { root.moveDay(dx || dy); return }
        if (dx !== 0) root.moveMonth(dx)
        if (dy !== 0) root.moveYear(dy)
      }
      onActivateRequested: root.goToToday()
      onCloseRequested: {
        if (root.daySchedule) root.daySchedule = false
        else root.close()
      }
      ...
      onTextKey: function(t) {
        ...
        else if (t === "w" || t === "W") root.daySchedule = !root.daySchedule
      }

      Flickable {
        id: calendarScroll
        anchors.fill: parent
        contentWidth: root.daySchedule ? scheduleColumn.width : calendarColumn.width
        contentHeight: root.daySchedule ? scheduleColumn.implicitHeight : calendarColumn.implicitHeight
        clip: true
        ...
        Column {
          id: scheduleColumn
          visible: root.daySchedule
          width: calendarScroll.width
          spacing: Style.space(16)

          Text {
            width: parent.width
            text: Qt.formatDateTime(root.scheduleDate, "dddd, MMMM d")
          ...
          Text {
            width: parent.width
            text: "Google Calendar isn’t connected yet.\nSign in to see your events here."
          ...
          Text {
            width: parent.width
            text: "← / →  Change day    T  Today    W / Esc  Month"
            wrapMode: Text.WordWrap
            color: root.contentForeground
            opacity: 0.6
            font.family: root.contentFontFamily
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        onClicked: {
                          var day = parent.modelData
                          root.scheduleDate = new Date(day.year, day.month, day.day)
                          root.daySchedule = true
                        }
                      }
